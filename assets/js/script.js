...
function handleSubmit(e) {
  ...
  const weigth = getInputNumberValue("weigth");
  const heigth = getInputNumberValue("heigth");
  const activityLevel = getSelectedValue("activity_level");
  
  if(gender === "selecione") return;
  if(!age) return;
  if(!weigth) return;
  if(!heigth) return;
  if(activityLevel === "selecione") return;

  const tmb = Math.round(
    gender === "female"
      ? (655 + (9.6 * weigth) + (1.8 * heigth) - (4.7 * age))
      : (66 + (13.7 * weigth) + (5 * heigth) - (6.8 * age))
  );
